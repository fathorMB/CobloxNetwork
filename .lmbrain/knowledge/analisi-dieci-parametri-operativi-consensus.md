...
- **4. Da quale grandezza dipende la proprietà voluta:**
  - La proprietà non è "la deriva sia una frazione ragionevole di `block_interval_ms`": è che la finestra di macinatura sul beacon di elezione resti troppo costosa da percorrere per intero. Il vincolo relazionale proposto dalla versione precedente — metà di `block_interval_ms`, cioè 2 500 ms — lascerebbe **2 500 candidati a un hash l'uno**, un ordine di grandezza dentro la banda quantificata da `docs/protocol/ledger.md`, §*"Challenge evidence"* (frase *"that window admits on the order of 10³–10⁶ legal values"*): **non morde la proprietà**.
...
- **2. Cosa ottiene un quorum sedente ai due estremi:**
  - *Portato al massimo:* Consente a server RPC o validatori disonesti di fornire prove di saldo obsolete (stale balance proofs), nascondendo all'utente spese, trasferimenti o burn avvenuti di recente. **Danno ulteriore, non solo sul saldo:** il passo 6 dell'algoritmo di verifica (`docs/protocol/ledger.md`, §*"Light-client balance verification"*) lega la stessa soglia all'accordo tra peer indipendenti, che è descritto letteralmente come *"an availability/fork alarm"* (frase completa: *"Peer agreement is an availability/fork alarm, never a substitute for proof verification"*). Alzare `max_current_balance_age_ms` non acceca solo la freschezza del saldo mostrato: acceca anche quell'allarme, perché tip più vecchi restano nella banda accettata più a lungo prima che un disaccordo fra peer venga segnalato.
...
- **5. Pavimento e liveness:** **Necessario, e già imposto oggi** — a differenza degli altri nove, questa è la sola riga dove il pavimento è già vincolo di genesi verificato: `core/coblox-core/src/params.rs`, funzione `check_relations`, regola `"min_revocation_effective_delay_blocks >= 1"`, impone `min_revocation_effective_delay_blocks >= 1`. Il pavimento dipende dal tempo minimo che i validatori superstiti richiedono per coordinarsi e committare un set successore conforme; a `F = 0` la catena rischia lo stallo che `docs/protocol/ledger.md`, §*"Revocation forces a validator set transition"*, frase *"the chain **stalls** at `effective_height` instead of finalizing blocks signed by a set containing a revoked key"*, descrive.
