...
- Android SDK command-line tools plus NDK `27.2.12479018`; API 26 is the
  minimum Android level because it is a widely supported baseline and is used
  in the generated arm64 linker target.
- JDK 17 and Gradle for the Android binding test.
...
Install the Android SDK command-line tools, NDK `27.2.12479018`, JDK 17, Gradle,
and `cargo-ndk` 3.5.4. Set `ANDROID_NDK_HOME` to the NDK directory, then run:
...
library only for the JVM test, generates Kotlin from the UDL, and executes
`coreVersion()`. The JVM test is intentional: it verifies the actual generated
binding and native FFI boundary without an emulator; the Android ABI itself is
cross-compiled separately. Do not substitute the host library for the packaged
Android `.so`.
...
The one-page shell invokes the Rust `core_version` Tauri command and displays
the returned `0.1.0`. Bundling and signing are intentionally excluded from this
specification.
