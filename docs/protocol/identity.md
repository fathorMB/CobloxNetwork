...
EnrollmentRequest = {
  ...
  "pow": {
    "algorithm": "sha256-leading-zero-bits-v0",
...
The active signed parameter set supplies `difficulty_bits` in the inclusive
range 18–40 and its own `parameter_set_hash`. A production network MUST choose a
value by benchmark and security review; `22` in the example is not a mainnet
default.

For nonce `n`, compute:

```text
pow_input = "coblox-enrollment-pow-v0\0"
          || chain_id_32
          || public_key_32
          ...
          || u64be(n)
pow_hash  = SHA-256(pow_input)
```

The proof is valid when the first `difficulty_bits` of `pow_hash`, read most
...
`2^difficulty_bits` SHA-256 evaluations and success probability after `h`
attempts is `1 - (1 - 2^-difficulty_bits)^h`.

Validators accept an enrollment only when all of these hold:

1. schema, signature, node-ID, Peer-ID/public-key binding, network ID, and
   timestamp (within the parameterized enrollment window) are valid;
2. the referenced parameter set is active at the referenced height and the
   stated difficulty exactly equals it;
3. `recent_block_id` is the locally finalized canonical block at
...
4. the proof hash meets the target;
5. neither `node_id` nor public key is already enrolled or revoked;
6. no pending request with the same key is being processed.

The proof cannot be copied to another identity, chain, parameter set, or recent
finalized window because all are hashed. Creating N key pairs is cheap, but registering them is
not free: every distinct key requires a fresh independent proof, so expected
cost grows linearly to `N × 2^difficulty_bits`. Parallel hardware can pay that
cost faster; proof of work is a cost, not proof of personhood. Admission rate
limits and validator monitoring are defense in depth and MUST NOT be counted as
the cryptographic Sybil guarantee.
...
at heights before the effective height. Key replacement is a fresh enrollment
...
Open alternatives are (a) a fixed `difficulty_bits` for an entire protocol
epoch, simple and predictable, or (b) bounded adjustment at epoch boundaries
using observed enrollment rate, more adaptive but manipulable. AGENT-007 owns
the security recommendation and the Project Lead/AGENT-002 own the signed
parameter governance. The algorithm, verification rules, 18–40 safety bounds,
and per-identity linear cost above are not draft.
